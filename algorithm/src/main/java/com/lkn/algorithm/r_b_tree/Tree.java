...
public class Tree<T extends Comparable> {
	...
	public synchronized void add(Node<T> node) {
		...
		node.setNodeType(Node.NodeType.RED);
		if (root == null) {
			node.toRoot();
			node.setNodeType(Node.NodeType.BLACK);
			root = node;
			return;
		}
		doAdd(root, node);
	 ...
	 */
	private void doAdd(Node<T> compareNode, Node<T> currNode) {
		directAdd(compareNode, currNode);
		// 调整树结构
		adjustTree(currNode);
	}

	private void adjustTree(Node<T> currNode) {
		if (currNode.isRoot()) {
	 ...
	 */
	private void adjustUncleRed(Node<T> currNode) {
		Node<T> parent = currNode.getParent();
	 ...
	 * @param currNode 当前节点
	 */
	private void adjustRightCurrentAndUncleBlack(Node<T> currNode) {
		Node<T> parent = currNode.getParent();
	 ...
	 * @param currNode	当前要插入的节点
	 */
	private void adjustLeftCurrentAndUncleBlack(Node<T> currNode) {
		Node<T> parent = currNode.getParent();
		if (currNode.inParentPosition() == Node.Position.LEFT) {
			// 要插入的节点是父节点的左孩子时
			Node<T> grandfather = parent.getParent();
	 ...
	 * @param node2	节点2
	 */
	private void changeNodeType(Node<T> node1, Node<T> node2) {
	 ...
	 * 直接添加
	 */
	private void directAdd(Node<T> compareNode, Node<T> currNode) {
