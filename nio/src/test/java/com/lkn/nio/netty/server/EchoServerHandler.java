...
public class EchoServerHandler extends ChannelInboundHandlerAdapter {
	...
	public void channelRead(ChannelHandlerContext ctx, Object msg) throws Exception {//每次收到消息时被调用
		...
		System.out.println(Thread.currentThread().getName() + " 服务器端收到消息: " + in);    //把消息打印到控制台
		ctx.writeAndFlush(Unpooled.copiedBuffer("已收到client消息内容", CharsetUtil.UTF_8));        //将收到的消息写入发送方，不刷新输出消息
	}
